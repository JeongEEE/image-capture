...
def add_file():
	files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*")), initialdir=r"C:/Users")
...
def browse_dest_path():
	...
	if forder_selected == '': # 사용자가 그냥 취소를 클릭햇을때
		return
...
def merge_image():
	try:
		...
		for idx, img in enumerate(images): # progress bar 적용
			# width가 원본이 아닐때에는 이미지 크기 조정이 필요
			if img_width > -1:
		...
		dest_path = os.path.join(txt_dest_path.get(), img_format)
